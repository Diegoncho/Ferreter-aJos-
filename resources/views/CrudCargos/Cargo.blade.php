...
@section('content')
    ...
    <div class="panel panel-default" style="min-width: 800px">
        <div class="panel-heading"><b>Listado de cargos</b></div>
        ...
        <div class="flexbox md-5">
            <div class="function-crud flex align-items-center">
                <form  action="{{ route('cargo') }}" class="form-search pull-left" method="GET" role="search">
                    <div class="form-group">
                        <input type="text" class="form-control" name="name" placeholder="Buscar cargo">
                    </div>
                    <button type="submit" class="btn btn-primary" >Buscar <i class="icon-search"></i></button>
                </form>


                <a href="{{ route('cargoAdd') }}" class="btn btn-success" style="margin:7px 0px;"><i class="icon-play_for_work "></i> Nuevo Registro</a>
            </div>

            <p>Hay <b>{{ $Cargos->total() }}</b> cargos</p>
        </div>
        ...
        <table class="table">
            ...
            @foreach($Cargos as $row)
                <tr data-id="{{ $row->id }}">
                    ...
                    <td>{{ $row->cargo }}</td>
                    <td>
                        <a href="{{ route('cargoEdit', $row->id) }}" class="btn btn-warning">Editar</a>
...
<form action="{{ route('cargo') }}/id" method="POST" id="form-delete">
    {{ csrf_field() }}
    {{ method_field('DELETE') }}                           
</form>
